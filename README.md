...
| **知识检索** | 语义 / 全文 / 倒排检索（Dify 检索模式可切换），Top-K 召回 |
| **提示词约束** | System Prompt 限定回答范围，防幻觉、防 AI 痕迹 |
...
└──────┬──────┘
       ...
       ▼
┌─────────────────┐
│  知识库检索节点  │  ← 向量/倒排索引（已向量化的个人文档）
│  (RAG Retriever)│     支持多路召回，Top-K 建议 8
└──────┬──────────┘
       │ 检索到的知识片段
       ▼
┌─────────────────┐
│   LLM 节点       │  ←  System Prompt（人设 + 约束 + 防幻觉 + 防 AI 痕迹）
...
2. **检索（Retrieval）**：用户提问时，检索节点将问题向量化，在索引中召回 Top-K 个最相关的文档片段（Top-K 建议调大到 8，召回更全、回答更稳）。
3. **提示词约束（Prompt Engineering）**：System Prompt 明确限定 Agent 身份、回答规则（只基于检索内容、不知道就说不知道、口语化、控制长度、**绝不暴露内部词**），有效防止大模型幻觉和穿帮。
4. **LLM 生成**：将检索到的知识片段 + 用户问题 + System Prompt 一起发送给大模型，生成最终回答。

> **⚠️ 重要经验**：提示词里引用知识检索结果**必须写节点 ID**（如 `{{#1788628000229.result#}}`），不能写节点标题（如 `{{#知识检索.result#}}`）——写标题会导致变量不解析，LLM 收不到知识库内容，表现为"明明有内容却总答记不太清"。完整排障见 [docs/troubleshooting.md](docs/troubleshooting.md)。
...
3. 知识检索节点关联刚创建的知识库，**Top-K 建议设为 8**
4. LLM 节点填入 [prompts/system-prompt-template.md](prompts/system-prompt-template.md) 中的提示词模板（v3.2）
5. **将提示词中的变量引用替换为检索节点的实际 ID**（不能写节点标题，见上文提示）
6. 选择 DeepSeek 模型
7. 保存并发布，获取 Web App 分享链接

### 6. 测试问答

在预览窗口或分享链接中提问（**先点"开启新对话"，一题一问**）：
...
- `你最不擅长什么？`（测试边界约束）
- `为什么没参加秋招？`（测试 L7 时间线口径，v3 新增）

> 测试须知：旧会话历史会污染新回答（前面答错的内容会带偏后续），必须"开启新对话"
> 再单题测试；批量粘贴多题会把整段当成一个 query，召回率骤降。详见
...
├── examples/
│   └── knowledge-base-sample.md       # 示例知识库（虚拟人物 L1-L7，已脱敏）
├── prompts/
│   └── system-prompt-template.md      # 脱敏后的系统提示词模板（v3.2）
├── docs/
│   ├── dify-deployment-guide.md       # Dify 本地 Docker 部署详细指南
│   ├── model-configuration.md         # 大模型配置说明（DeepSeek / OpenAI 兼容）
│   └── troubleshooting.md             # 排障指南（召回断裂三步走/测试方法论/内部词泄漏）
└── screenshots/
...
本项目的核心技术亮点之一是**通过提示词工程约束大模型行为**，防止幻觉、防止穿帮。关键约束（v3.2）：

1. **身份锚定**：明确 Agent 是"某个人的数字分身"，用第一人称回答
2. **知识边界**：只回答知识库中有依据的内容，不知道就自然说"这个细节我记不太清，我们见面可以展开聊"（不解释原因）
3. **一致性约束**：回答口径必须与知识库（简历）完全一致，不新增、不夸大、不编造
4. **风格控制**：口语化、真诚、避免 AI 腔，回答控制在 2-4 句话
5. **敏感信息保护**：不主动透露联系方式、薪资等敏感信息
6. **软素质兜底（v2 新增）**：软素质类问题（抗压、协作、缺点、职业规划）优先从知识库 L6 层提取现成答案
7. **数字证据优先（v2 新增）**：回答优先引用量化成果（准确率、耗时、吞吐、排名）
8. **追问闭环（v2 新增）**：技术细节追问时引用 L2 经历的"可追问细节"
9. **时间线敏感口径（v3 新增）**：被问"为什么没参加秋招"等，按知识库 L7 层口径回答，不承认"提前准备"
10. **内部词铁律（v3.2 新增）**：回答里绝不出现"定稿、话术、口径、知识库、检索、提前准备、模板"等词——防止被面试官识破是预设答案
11. **变量引用用节点 ID（v3.2 修复）**：知识库注入必须写检索节点的实际 ID，写标题会导致召回断裂

知识库采用 **L1-L7 分层结构**（身份定位 / 经历 / 硬指标 / 技能 / 亮点 / 面试问答 / 时间线口径），完整模板见 [prompts/system-prompt-template.md](prompts/system-prompt-template.md)，示例知识库见 [examples/knowledge-base-sample.md](examples/knowledge-base-sample.md)。
...
### Q: 知识库明明有内容，却总答"记不太清"？
A: 最常见的根因是**提示词里变量引用写成了节点标题**（`{{#知识检索.result#}}`）而不是节点 ID。按 [docs/troubleshooting.md](docs/troubleshooting.md) 的"排查三步走"逐层验证：召回测试 → 检索节点运行状态 → LLM 节点收到的实际内容。

### Q: 测试时回答不稳定，前面好后面差？
A: 会话历史污染。每道题前点"开启新对话"，一题一问，不要批量粘贴。

### Q: 回答里出现"定稿/话术/知识库"等词？
A: 内部词泄漏。确认提示词 v3.2 规则 12（内部词铁律）已生效，时间线问题确认规则 10 + 知识库 L7 层存在。
...
- **云端部署**：将 Dify 部署到云服务器（约 ¥50-100/月），给 HR 提供公网访问地址
